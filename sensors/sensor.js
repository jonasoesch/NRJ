...
// Get all the Plug ids from the Api
http.get(options, function(res) {
    ...
    // Get the ids from the JSON string
    res.on('end', function(){
      var json = JSON.parse(pageData);
      
      plugs = [];
      for(var i=0; i<json.length; i++) {
      	plugs.push(json[i].plugId);
      }
      // call the function that sends the periodical data
      postConsumptions(plugs);
    });
}).on('error', function(e) {
...
function postConsumptions(plugs) {
	setInterval(function() {

	    // Generate Plug Id
	    var plug_i = numberBetween(0,(plugs.length-1));

	    // Generate kW
	    var kW = numberBetween(1,20);

	    // Generate Date in the right format
	    var date = (new Date()).toString("yyyy-MM-dd'T'HH:mm:ss:SSS");
